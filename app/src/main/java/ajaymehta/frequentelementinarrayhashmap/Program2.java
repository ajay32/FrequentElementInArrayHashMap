...
// this is the program with for each loop ..to store elements in hash map
...
public class Program2
{
    static void getMostFrequentElement(int inputArray[])
    {
        ...
        for (int i : inputArray)  // this is the program with for each loop ..to store elements in hash map
        {
            if (elementCountMap.containsKey(i))
            {
                //If an element is present, incrementing its count by 1

                elementCountMap.put(i, elementCountMap.get(i)+1);  // getting key of element i [ i is an array element]
            }
            else
            {
    ...
    {
        getMostFrequentElement(new int[]{4,4,4,4, 5, 8, 7, 4, 7, 6,7});
